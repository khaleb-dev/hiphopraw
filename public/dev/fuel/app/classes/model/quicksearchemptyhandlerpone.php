...
namespace Model;
use DB;
class Quicksearchemptyhandlerpone extends \Model {
	...
	{
		$id = Quicksearchemptyhandlerpone::get_profile_id($username, $password);
	...
	public static function get_yourmatchfirstage($username, $password,$chosen1,$chosen2,$chosen3,$chosen4,$chosen5,$myactual1,$myactual2,$myactual3,$myactual4,$myactual5,$actual11,$actual21,$actual31,$actual41,$actual51)
	{
		...
		$end = 99999999999;
		$topmatchchoice = Quicksearchemptyhandlerpone::get_top_match_choice($username, $password);
	if($topmatchchoice == 'Three per week')
		...
		if(!empty($chosen2) && empty($chosen3) && empty($chosen4) && empty($chosen5))
		{
			...
			if(!empty($chosen3) && empty($chosen2) && empty($chosen4) && empty($chosen5))
			{
				$result1 = DB::select('profiles.created_at','profiles.id','city','first_name','last_name','picture','profiles.user_id','state','username','group_id','email')
				->from('profiles')
				->join('users', 'right')->on('users.id', '=', 'profiles.user_id')
			...
			if(!empty($chosen4) && empty($chosen3) && empty($chosen2) && empty($chosen5))
			{
				$result1 = DB::select('profiles.created_at','profiles.id','city','first_name','last_name','picture','profiles.user_id','state','username','group_id','email')
				->from('profiles')
				->join('users', 'right')->on('users.id', '=', 'profiles.user_id')
